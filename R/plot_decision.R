...
PlotStackedBar <- function(data,
  ...
  p1 <- dat1 %>%
    ggplot(aes(xmin = xl+FTAScore*2, xmax = xh+FTAScore*2,
               ymin = yl, ymax = yh,
               group = as.factor(D), fill=as.factor(D))) +
    geom_rect() +
    scale_fill_manual(values=d.colors,
                      labels = d.labels,
    ...
    scale_x_continuous(breaks = round(unique((dat1$xh-dat1$xl)/2+dat1$xl+dat1$FTAScore*2)),
  ...
  p2 <- dat2 %>%
    ggplot(aes(xmin = xl+NCAScore*2, xmax = xh+NCAScore*2,
               ymin = yl, ymax = yh,
               group = as.factor(D), fill=as.factor(D))) +
    geom_rect() +
    scale_fill_manual(values=d.colors,
    ...
    theme(legend.position=legend.position, 
          ...
          legend.text = element_text(size = 20),
          legend.title = element_text(size = 20)) +
    scale_x_continuous(breaks = round(unique((dat2$xh-dat2$xl)/2+dat2$xl+dat2$NCAScore*2)),
  ...
  p3 <- dat3 %>%
    ggplot(aes(xmin = xl+NVCAFlag*2, xmax = xh+NVCAFlag*2,
               ymin = yl, ymax = yh,
               group = as.factor(D), fill=as.factor(D))) +
    geom_rect() +
    scale_fill_manual(values=d.colors,
    ...
    theme(legend.position=legend.position, 
          ...
          legend.title = element_text(size = 20),
          legend.text.align = 0) +
    scale_x_continuous(breaks = round(unique((dat3$xh-dat3$xl)/2+dat3$xl+dat3$NVCAFlag*2)),
...
PlotStackedBarDMF <- function(data,
  ...
  p1 <- dat1 %>%
    ggplot(aes(xmin = xl+dmf*2, xmax = xh+dmf*2,
               ymin = yl, ymax = yh,
               group = as.factor(D), fill=as.factor(D))) +
    geom_rect() +
    scale_fill_manual(values=d.colors,
    ...
    theme(legend.position=legend.position, 
          ...
          legend.title = element_text(size = 20),
          legend.text.align = 0) +
    scale_x_continuous(breaks = round(unique((dat1$xh-dat1$xl)/2+dat1$xl+dat1$dmf*2)),
                       labels = dmf.category) +
    scale_y_continuous(breaks = c(0, 25, 50, 75, 100),
                       labels = paste0(c(0, 25, 50, 75, 100), "%")) +
    labs(title = "DMF\nRecommendation") 
  ...
  p2 <- dat2 %>%
    ...
    theme(legend.position=legend.position, 
          ...
          legend.title = element_text(size = 20),
          legend.text.align = 0) +
    scale_x_continuous(breaks = round(unique((dat2$xh-dat2$xl)/2+dat2$xl+dat2$dmf*2)),
                       labels = dmf.category) +
    scale_y_continuous(breaks = c(0, 25, 50, 75, 100),
                       labels = paste0(c(0, 25, 50, 75, 100), "%")) +
    labs(title = "DMF\nRecommendation") 
